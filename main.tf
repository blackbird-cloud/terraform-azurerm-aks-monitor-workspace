...
resource "azurerm_monitor_data_collection_endpoint" "dce" {
  name                = "MSProm-${var.resource_group_location}-${var.monitor_workspace_name}"
  resource_group_name = var.resource_group_name
  location            = var.resource_group_location
  kind                = "Linux"
}
...
# All below are dynamic to allow to add multiple clusters
resource "azurerm_monitor_data_collection_rule_association" "dcra" {
  for_each = var.clusters

  name                    = "MSProm-${var.resource_group_location}-${each.key}"
...
resource "azurerm_monitor_alert_prometheus_rule_group" "node_recording_rules_rule_group" {
  ...
  name                = "NodeRecordingRulesRuleGroup-${each.key}"
  location            = var.resource_group_location
  resource_group_name = var.resource_group_name
  ...
  interval            = "PT1M"
  scopes              = [azurerm_monitor_workspace.amw.id, each.value]
...
resource "azurerm_monitor_alert_prometheus_rule_group" "kubernetes_recording_rules_rule_group" {
  ...
  name                = "KubernetesRecordingRulesRuleGroup-${each.key}"
  location            = var.resource_group_location
  resource_group_name = var.resource_group_name
  cluster_name        = each.key
